...
def calculate_zoom_level(df):
    ...
    zoom_levels = np.interp(np.log(speed_filtered + 1),
                            [0, np.log(df['speed_filtered'].max() + 1)], [13, 7])
...
min_max = fin.groupby('group', as_index=False).agg(
    {'timestamp': ['min', 'max'], 'distance': 'sum'})
...
fin = fin.merge(
    min_max[['group', 'duration', 'distance']], on='group', how='left')

# merge data
df_new = df.merge(fin[['timestamp', 'all_dts', 'lon', 'lat',
                  'distance', 'speed', 'duration', 'types_new']], on='timestamp')
...
df_new.loc[(df_new['types_new'].isin(['CYCLING', 'WALKING', 'IN_FERRY', 'sp'])
            == False) & (df_new['duration'] > 2.5), 'zoom_level_run'] = 8
...
flights['started_at'] = pd.to_datetime(
    flights['started_at']).dt.tz_localize(None)
flights['finished_at'] = pd.to_datetime(
    flights['finished_at']).dt.tz_localize(None)
flights['started_hour'] = flights['started_at'].apply(
    lambda x: x.replace(microsecond=0, second=0, minute=0))
flights['finished_hour'] = flights['finished_at'].apply(
    lambda x: x.replace(microsecond=0, second=0, minute=0))
flights['finished_hour'] = flights['finished_hour'] + timedelta(hours=1)

df_chan = df_new
df_chan['timestamp'] = pd.to_datetime(
    df_chan['timestamp']).dt.tz_localize(None)
...
for i, obj in flights.iterrows():
    ...
    df_chan = df_chan.loc[(df_chan['timestamp'] <= started)
                          | (df_chan['timestamp'] >= finished)]
...
df['end_zoom'] = df['start_zoom'].shift(-1)
...
for i, obj in df.iterrows():
    ...
    js['end_lon'] = obj['end_lon']
    js['end_lat'] = obj['end_lat']

    js['duration'] = obj['duration']

    js['start_zoom'] = obj['start_zoom']
    js['end_zoom'] = obj['end_zoom']
